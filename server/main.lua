...
function P.EnsureTables()
    ...
    pcall(function()
        ...
        MySQL.query.await(('ALTER TABLE `%s` ADD COLUMN IF NOT EXISTS `removed_at` TIMESTAMP NULL DEFAULT NULL'):format(Config.BanTable))
        MySQL.query.await(('ALTER TABLE `%s` ADD COLUMN IF NOT EXISTS `removed_by` VARCHAR(255) NULL DEFAULT NULL'):format(Config.BanTable))
        MySQL.query.await(('ALTER TABLE `%s` ADD COLUMN IF NOT EXISTS `remove_reason` VARCHAR(255) NULL DEFAULT NULL'):format(Config.BanTable))
